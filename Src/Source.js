...
Source.vsSource = `
    ...
    //----------Directional Lights-----------
    uniform int uNumbDirectionalLights;
	uniform vec3 uDirectionalLights[MAX_LIGHTS];
	varying highp vec3 vDirectionalLights;

	//----------Point Lights-----------
	uniform int uNumbPointLights;
	uniform mediump vec3 uPointLights[MAX_LIGHTS];
	varying mediump vec3 vSurfaceToLight[MAX_LIGHTS];
	...
	void main(void) {
		...
		for(int i = 0; i < MAX_LIGHTS; i++) {
		    ...
		    if(i < 1) {
               dir = max(dot(vNormal, normalize(uDirectionalLights[i])), 0.0);
		    }
		    // Render All Point Lights
			if(i < uNumbPointLights) {
				vSurfaceToLight[i] = uPointLights[i] - vec3(vertexPosition);
			}
		}

        vDirectionalLights = vec3(1, 1, 1) * dir;
...
Source.fsSource = `
    ...
    uniform mediump vec3 uAmbientColor;
	...
	void main(void) {
	    mediump vec4 finalColor = vec4(0, 0, 0, 1);

	    mediump vec3 N = normalize(vNormal);
        mediump float lam = 0.0;

        for(int i = 0; i < MAX_LIGHTS; i++) {
			if(i < uNumbPointLights) {
                lam = dot(N, normalize(vSurfaceToLight[i]));

                if(lam > 0.05) {
                    finalColor += vec4(1, 1, 1, 1) * vColor * lam;
                }
			}
		}
        ...
        gl_FragColor += vec4(vDirectionalLights, 1.0);
