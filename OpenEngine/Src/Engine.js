...
Engine.prototype.initialize = function() {
	...
	this.defaultShaderProgramInfo = OpenEngine.Shaders.createShaderProgram(this.GetActiveGLContext(), OpenEngine.Shaders.vsSource, OpenEngine.Shaders.fsSource, Engine.DEFAULT_SHADER);
	this.textureShaderProgramInfo = OpenEngine.Shaders.createShaderProgram(this.GetActiveGLContext(), OpenEngine.Shaders.vsTextureSource, OpenEngine.Shaders.fsTextureSource, Engine.TEXTURE_SHADER);
...
Engine.prototype.SetEngineLights = function() {
	...
	this.DirectionalLightDataColors = [];
	this.PointLightData = [];
	this.PointLightDataColors = [];
	for(let i = 0; i < this.Lights.length; i++) {
		if(this.Lights[i]._isDirectionalLight) {
			...
			this.DirectionalLightDataColors = this.DirectionalLightDataColors.concat(this.Lights[i].color.toArray());
		} else if(this.Lights[i]._isPointLight) {
			this.PointLightData = this.PointLightData.concat(this.Lights[i].position.toArray());
			this.PointLightDataColors = this.PointLightDataColors.concat(this.Lights[i].color.toArray());
		}
	}
};
...
Engine.prototype.RenderMesh = function(Mesh, matrix) {
	...
	if(this.DirectionalLightData.length > 0) {
		this.gl.uniform3fv(programInfo.uniformLocations.directionalLights, this.DirectionalLightData);
		this.gl.uniform3fv(programInfo.uniformLocations.directionalLightColors, this.DirectionalLightDataColors);
	}
	
	/* Set ALL Active Lights In Engine (all active lights effect mesh) (if any) */
	this.gl.uniform1i(programInfo.uniformLocations.numbPointLights, this.PointLightData.length);
	if(this.PointLightData.length > 0) {
		this.gl.uniform3fv(programInfo.uniformLocations.pointLights, this.PointLightData);
		this.gl.uniform3fv(programInfo.uniformLocations.pointLightColors, this.PointLightDataColors);
	}
	...
	this.gl.uniformMatrix4fv(programInfo.uniformLocations.worldMatrix, false, this.WorldMatrix.matrix);
